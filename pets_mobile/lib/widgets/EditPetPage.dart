...
class _EditPetPageState extends State<EditPetPage> {
  ...
  void initState() {
    ...
    if (breedProvider.breeds.isNotEmpty) {
      try {
        _selectedBreedId = breedProvider.breeds.firstWhere((b) => b.name == widget.pet.breedName).id;
      } catch (e) {
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        await updatePet(widget.pet.id, petRequest, _imageFile, widget.token);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Pet updated successfully!'), backgroundColor: Colors.green));
          Navigator.of(context).pop(true);
        }
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to update pet: $e'), backgroundColor: Colors.red));
        }
      } finally {
              ...
              children: [
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: _imageFile != null
                        ...
                        : ClipRRect(borderRadius: BorderRadius.circular(8.0), child: Image.network(widget.pet.imageUrl, fit: BoxFit.cover, errorBuilder: (c, e, s) => const Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.camera_alt), Text('Select a new photo')]))),
                  ),
                ),
                ...
                Consumer<BreedProvider>(
                  builder: (context, breedProvider, child) {
                    ...
                    if (breedProvider.error != null) return Center(child: Text('Error: ${breedProvider.error}'));
